...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="mx-auto min-h-dvh w-full max-w-2xl bg-zinc-950/30 shadow-2xl">
    ...
    <header className="sticky top-0 z-30 flex h-16 items-center justify-between bg-zinc-950/75 px-5 pt-[env(safe-area-inset-top)] backdrop-blur-2xl">
      <Link href="/home" className="flex items-center gap-2 font-semibold"><Scissors className="size-5 text-amber-300" />BarberApp</Link>
      <button onClick={() => setNotificationsOpen(true)} aria-label="Attiva o disattiva notifiche" aria-haspopup="dialog" className="grid size-11 place-items-center rounded-full bg-white/[.055] text-zinc-200 transition hover:bg-white/10"><Bell className="size-5" /></button>
    </header>
    <AnimatePresence mode="wait" initial={false}>
      <motion.main key={pathname} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -5 }} transition={{ duration: .2, ease: "easeOut" }} className="px-5 pb-[calc(7.5rem+env(safe-area-inset-bottom))] pt-7">{children}</motion.main>
    </AnimatePresence>
