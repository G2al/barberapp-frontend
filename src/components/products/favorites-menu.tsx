...
export function FavoritesMenu({ open, onOpen, onClose }: { open: boolean; onOpen: () => void; onClose: () => void }) {
  ...
  const whatsappPhone = config.data?.phone?.replace(/\D/g, "") ?? "";
  const whatsappMessage = `Ciao Mottola's Family, sono interessato ${products.length === 1 ? "a questo prodotto" : "a questi prodotti"}:\n\n${products.map((product) => `• ${product.name}`).join("\n")}\n\nPotrei avere maggiori informazioni su disponibilità e prezzo?`;
