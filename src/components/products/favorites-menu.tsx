...
export function FavoritesMenu({ open, onOpen, onClose }: { open: boolean; onOpen: () => void; onClose: () => void }) {
  ...
  const whatsappMessage = `Ciao Mottola's Family, vorrei ordinare ${products.length === 1 ? "questo prodotto" : "questi prodotti"}:\n\n${products.map((product) => `• ${product.name} — ${euro(product.price)}`).join("\n")}\n\nPotete confermarmi disponibilità e totale?`;
  const whatsappUrl = `https://wa.me/393669772648?text=${encodeURIComponent(whatsappMessage)}`;
